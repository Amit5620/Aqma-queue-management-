...
const TokenForm = () => {
  ...
  useEffect(() => {
    ...
    const fetchVendorItems = async (vendorId, token) => {
      try {
        const res = await fetch(`http://localhost:8000/api/vendor/${vendorId}/items`, {
          headers: { Authorization: `Bearer ${token}` }
        });
    ...
    const fetchCabins = async (vendorId, token) => {
      try {
        const res = await fetch(`http://localhost:8000/api/cabins`, {
          headers: { Authorization: `Bearer ${token}` }
        });
    ...
    const fetchCollectionStats = async (token) => {
      try {
        const res = await fetch(`http://localhost:8000/api/tokens/collection-stats`, {
          headers: { Authorization: `Bearer ${token}` }
        });
